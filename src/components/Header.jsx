...
export default function Header() {
  const { dark, darkModeToggle } = useSearchContext();
  return (
    <header className="flex justify-between items-center md:mb-[35px] mb-4">
      <section>
        <span className="text-[26px] font-bold text-[#222731] dark:text-pure-white">
          devfinder
        </span>
      </section>
      <section>
        <div
          className="flex h-[20px] gap-4 text-slate-gray cursor-pointer select-none"
          onClick={darkModeToggle}
        >
          <span className="uppercase text-[13px] font-bold tracking-[2.5px] dark:text-pure-white">
            {dark === true ? "light" : "dark"}
          </span>
          {dark ? (
            <svg
              className="fill-slate-gray dark:fill-pure-white"
              width="20"
              height="20"
